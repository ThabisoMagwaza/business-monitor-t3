    ...
    <Page>
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-900 flex items-center gap-1">
                ...
                <span className="truncate">
                  {new Date(receipt.scanCreatedAt).toLocaleDateString('en-ZA', {
                    day: '2-digit',
                    month: 'short',
                    year: 'numeric',
                  })}
      ...
      <Card className="bg-white">
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <CardTitle>
            Transactions ({receipt.scanResult?.items.length})
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          {receipt.scanResult?.items.map((item, index) => (
            <div key={index}>
              ...
              {index < (receipt.scanResult?.items.length ?? 0) - 1 && (
                <Separator className="mt-4" />
              )}
            </div>
          ))}

          {receipt.scanResult?.items.length &&
            receipt.scanResult?.items.length > 0 && (
              <>
                <Separator className="my-4" />
                <div className="flex justify-between items-center pt-2">
                  <p className="text-sm font-semibold">Total Amount</p>
                  <p className="text-sm font-bold">
                    {formatCurrencyAmount(receipt.totalAmount)}
                  </p>
                </div>
              </>
            )}
